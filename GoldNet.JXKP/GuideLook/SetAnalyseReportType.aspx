<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SetAnalyseReportType.aspx.cs" Inherits="GoldNet.JXKP.GuideLook.SetAnalyseReportType" %>
        ...
        */
        function UpdateMenuButton(node) {
            ...
            if (selNodeId == "0") {
                ...
                Menu_add.setDisabled(false);
                Menu_del.setDisabled(true);
                Menu_edit.setDisabled(true);
                Menu_up.setDisabled(true);
                Menu_down.setDisabled(true);
                return;
            }
            btn_Edit.setDisabled(false);
            Menu_edit.setDisabled(false);
            btn_Del.setDisabled(false);
            Menu_del.setDisabled(false);
            //二级节点
        ...
        */
        function nodeOpCallback (btn, text, optype, node) {
            ...
            if (optype == "1") {
                ...
                var cloneid = parseInt(clonedMaxNode.id) + 1;
...
<body>
    ...
    <ext:ScriptManager ID="ScriptManager1" runat="server" >
        <Listeners>
            <DocumentReady Handler="TreeCtrl.root.select(); " />
        </Listeners>
    </ext:ScriptManager>
    <div>
        <ext:ViewPort ID="ViewPort1" runat="server">
            <Body>
                <ext:BorderLayout ID="BorderLayout2" runat="server">
                    <Center>
                        <ext:TreePanel runat="server" Header="false" Width="400" ID="TreeCtrl" EnableDD="true" UseArrows="true"
                            ...
                            <Listeners>
                                <BeforeNodeDrop Fn="TreeBeforeNodeDrop" />
                                <DragDrop Fn = "TreeDragDropHandler" />
                                <ContextMenu Fn="TreeContextMenu" />
        ...
        <ext:KeyNav ID="KeyNav1" runat="server" Target="#{TreeCtrl}" > 
            <Left   Handler= "TreeNodeSelected();" />
            <Right  Handler= "TreeNodeSelected();" />
            <Up     Handler= "TreeNodeSelected();" />
            <Down   Handler= "TreeNodeSelected();" />
        </ext:KeyNav>
    </div>
    <div class="x-hide-display">
        <ext:Menu runat="server" ID="treeMenu">
            <Items>
                <ext:MenuItem ID="Menu_add" runat="server" Icon="Add" Text="添加子类别">
                    <Listeners>
                        <Click Handler="TreeOpration(1);" /> 
                    </Listeners>
                </ext:MenuItem>
                <ext:MenuItem ID="Menu_edit" runat="server" Icon="FolderEdit" Text="类别重命名">
                    <Listeners>
                        <Click Handler="TreeOpration(2);" /> 
                    </Listeners>
                </ext:MenuItem>
                <ext:MenuItem ID="Menu_del" runat="server" Icon="Delete" Text="删除类别">
                    <Listeners>
                        <Click Handler="TreeOpration(3);" /> 
                    </Listeners>
                </ext:MenuItem>
                <ext:MenuItem ID="Menu_up" runat="server" Icon="ArrowUp" Text="上移">
                    <Listeners>
                        <Click Handler="TreeOpration(4);" /> 
                    </Listeners>
                </ext:MenuItem>
                <ext:MenuItem ID="Menu_down" runat="server" Icon="ArrowDown" Text="下移">
                    <Listeners>
                        <Click Handler="TreeOpration(5);" /> 
                    </Listeners>
                </ext:MenuItem>
