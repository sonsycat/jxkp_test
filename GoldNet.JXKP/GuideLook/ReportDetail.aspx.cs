...
namespace GoldNet.JXKP.GuideLook
{
    public partial class ReportDetail : System.Web.UI.Page
    {
        ...
        public string RefreshMenu()
        {
            ...
            DataTable l_TreeDt = dal.getSetReportGuideTreeBuilder(DeptType, Session["OrganType"].ToString()).Tables[0];
        ...
        protected void Next_Click(object sender, AjaxEventArgs e)
        {
            ...
            //保存选择指标，选择完人员科室存入数据库中
            Session["SelectRightGuideItem"] = CreateTableForSelectRightItem(items2);

            string OrganType = Session["OrganType"].ToString();

            //判断人的情况和科室情况
            if (OrganType.Equals("R"))
            {
                Response.Redirect("SetStation.aspx?rptid=" + m_rptid + "");
            }
            else
            {
                Response.Redirect("SetDept.aspx?rptid=" + m_rptid + "");
            }
        ...
        protected void BindLeftSelector(string bsc, string depttype, string tag, string selectedid)
        {
            ...
            DataTable dt = dal.getReportSearchGuide(Session["OrganType"].ToString(), this.Combo_DeptType.SelectedItem.Value.ToString(), bsc, tag).Tables[0];
