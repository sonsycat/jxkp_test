...
{
    public partial class DeptBudget : PageBase
    {
        ...
        {
            string month = this.Comb_StartMonth.SelectedItem.Value.Length == 2 ? this.Comb_StartMonth.SelectedItem.Value : "0" + this.Comb_StartMonth.SelectedItem.Value;
            string dd1 = this.Comb_StartYear.SelectedItem.Value + month;
            month = this.Comb_EndMonth.SelectedItem.Value.Length == 2 ? this.Comb_EndMonth.SelectedItem.Value : "0" + this.Comb_EndMonth.SelectedItem.Value;
            string dd2 = this.Comb_EndYear.SelectedItem.Value + month;
            Currdt = dal.GetDeptBudget(dd1.Replace("-", ""), dd2.Replace("-", ""),this.DeptFilter("")).Tables[0];
        ...
        protected void GetQueryPer(object sender, AjaxEventArgs e)
        {
            ...
            if (selectRow == null || selectRow[0]["科室编码"] == "999999")
            {
                return;
            }
            PageResponse(this.getPageResponseUrl("PerBudget.aspx", dd1.Replace("-", ""), dd2.Replace("-", ""), selectRow[0]["科室编码"].ToString()), "科室收治情况---" + selectRow[0]["科室名称"].ToString());
        }
        ...
        protected void OutExcel(object sender, EventArgs e)
        {
            ...
            ExcelTable = Currdt;
            ex.ExportToLocal(ExcelTable, this.Page, "xls", "全院收治情况");
        }
        ...
        private string getPageResponseUrl(string PageUrl, string fromDate, string toDate, string deptCode)
        {
            string ResponseUrl = "" + PageUrl + "?DeptCode=" + deptCode + "&FromDate=" + fromDate + "&ToDate=" + toDate;
            return ResponseUrl;
        }
