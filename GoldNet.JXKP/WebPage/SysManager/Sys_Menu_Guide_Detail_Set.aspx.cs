...
{
    public partial class Sys_Menu_Guide_Detail_Set : PageBase
    {
        #region --页面初始化--
        /// <summary>
        /// 选择报表ID
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Ext.IsAjaxRequest)
            {
                ...
                DataTable Adb = dal.GetFiledaccount();
                Store4.DataSource = Adb;
                Store4.DataBind();

                DataTable Bdb = dal.GetGuideByappid(Request["APP_MENU_ID"].ToString());
                Store3.DataSource = Bdb;
                Store3.DataBind();
        ...
        protected void Selectedguide(object sender, AjaxEventArgs e)
        {
            this.Store2.RemoveAll();
            SE_ROLE dal = new SE_ROLE();
            DataTable tb = dal.getReportGuidebyappid(Request.QueryString["APP_MENU_ID"].ToString(),this.comguide.SelectedItem.Value).Tables[0];
            this.accounttype.SelectedItem.Value = dal.getGuidebyappid(Request.QueryString["APP_MENU_ID"].ToString(), this.comguide.SelectedItem.Value);
            if (tb.Rows.Count > 0)
            {
                this.Store2.DataSource = tb;
        ...
        {
            if (this.comguide.SelectedItem.Value != "" && this.accounttype.SelectedItem.Value != "")
            {
                ...
                try
                {
                    dal.SaveSysmenuguidedetail(tb, rptid, this.comguide.SelectedItem.Value, this.accounttype.SelectedItem.Value);
