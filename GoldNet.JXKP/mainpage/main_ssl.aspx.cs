...
{
    public partial class main_ssl : System.Web.UI.Page
    {
        ...
        private DataTable GetStoreData()
        {
            ...
            DataTable dt = GetSSL(deptcode, stationcode, personid, yearstr);

            //string[] names = { "手术总例数", "同期手术总例数", "大手术例数", "同期大手术例数" };
            //double[] values = { Convert.ToDouble(dt.Rows[0]["sszls"].ToString()), Convert.ToDouble(dt.Rows[0]["tqsszls"].ToString()), Convert.ToDouble(dt.Rows[0]["dsszls"].ToString()), Convert.ToDouble(dt.Rows[0]["tqdsszls"].ToString()) };
            if (dt.Rows.Count > 0)
            {
                ...
                c.CreateStatisticPic(this.Chartssl);
        ...
        /// <returns></returns>
        public DataTable GetSSL(string deptcode, string stationcode, String personid, String yearst)
        {
            HomeDal dal = new HomeDal();
            DataTable dt = dal.GetMzl(deptcode, stationcode, personid, yearst, "04").Tables[0];
            return dt;
        }
    }
}
