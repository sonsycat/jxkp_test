...
{
    public partial class main_kpi : System.Web.UI.Page
    {
        /// <summary>
        /// 初始化处理
        /// </summary>
        ...
        private DataTable GetStoreData()
        {
            ...
            DataTable dt = dal.GetGuidess(deptcode, stationcode, curguide, yearstr, personid).Tables[0];

            //获取指标数据后绘制图表
            GuagePic pic = new GuagePic();
            string temp = dt.Rows.Count == 0 ? "0" : dt.Rows[0]["WCBFB"].ToString();
            pic.BuideGuagePic(dt.Rows.Count > 0 ? Convert.ToString(Convert.ToDouble(temp) * 100) : "0");
            string guidname = "0";
            string guidmbz = "0";
            string guidwcz = "0";
            string guidwcbfb = "0";
