...
namespace GoldNet.JXKP.Templet.BLL.Fields
{
    ...
    /// </summary>
    [Serializable()]
    internal class CharModifyField : IFieldType
    {
        #region --私有变量--
        ...
        #region --构造函数--
        public CharModifyField(Field field)
        {
            _field = field;
        }
        ...
        public string ListDisplayDataName
        {
            get
            {
                return "CHARMODIFY_" + _field.ID.ToString();
            }
        }
        ...
        public int InsertSpecialPropertyFormPage(System.Web.UI.Page curPage, ref string fieldDefineSql)
        {
            ...
            fieldDefineSql = " ALTER TABLE $$TABLE_NAME$$ ADD CHARMODIFY_" + this._field.ID.ToString()
        ...
        public void UpdateSpecialPropertyFormPage(System.Web.UI.Page curPage, ref string fieldDefineSql)
        {
            ...
            fieldDefineSql = " ALTER TABLE $$TABLE_NAME$$ modify  CHARMODIFY_" + this._field.ID.ToString()
                + " VARCHAR2(" + charInfo.MaxLen + ") ";
        }
        //		}
        ...
        public void ShowInputControl(System.Web.UI.Control curCtrl, bool pass)
        {
            ...
            textInput.ID = "CHARMODIFYINPUT_" + Encrypt.EncryptMyStr("iloveyou", this._field.ID.ToString());
        ...
        {
            CharFieldDefineInfo charInfo = this.charDefineInfo;
            string sql = string.Format("UPDATE {0}." + tabName + " SET CHARMODIFY_" + this._field.ID.ToString() + " = ? WHERE (ID = ?)",DataUser.ZLGL);

            string newValue = ((TextBox)curPage.FindControl("CHARMODIFYINPUT_" + Encrypt.EncryptMyStr("iloveyou", _field.ID.ToString()))).Text;
        ...
        {
            string sql = string.Format("SELECT CHARMODIFY_" + this._field.ID.ToString() + " FROM {0}." + tabName + " WHERE (ID = ?)",DataUser.ZLGL);
        ...
        public void ShowEditControl(string tabName, int recID, System.Web.UI.Control curCtrl,bool pass)
        {
            ...
            // CharFieldDefineInfo charInfo = getCharFieldInfoByID(_field.FieldDefineID);

            TextBox textInput = new TextBox();
            textInput.ID = "CHARMODIFYINPUT_" + Encrypt.EncryptMyStr("iloveyou", this._field.ID.ToString());
            textInput.MaxLength = this.charDefineInfo.MaxLen;
            textInput.Text = val;
            textInput.Width = new Unit("280px");
            if (!pass)
            {
            ...
            if (this.charDefineInfo.MultiLine)
            {
                textInput.TextMode = TextBoxMode.MultiLine;
                textInput.Rows = this.charDefineInfo.DisplayLine;
            }
            else
                textInput.TextMode = TextBoxMode.SingleLine;

            curCtrl.Controls.Add(textInput);
            if (this.charDefineInfo.ValueRequired)
            {
                Goldnet.Ext.Web.Label lb = new Goldnet.Ext.Web.Label();
                lb.Text = "*";
                lb.ForeColor = System.Drawing.Color.Red;
            ...
            if (this.charDefineInfo.Common != "")
                curCtrl.Controls.Add(new LiteralControl("<br><span class='gs-input-desc'>" + CleanString.InputText(this.charDefineInfo.Common, 500).Replace("\n", "<br>") + "</span>"));
        }
        ...
        {
            string sql;
            sql = string.Format("INSERT INTO {0}.T_CHARMODIFYFIELDDICT (id,ValueRequired, MultiLine, DisplayLine, MaxLen, Common, DefaultValue) VALUES (?, ?,?,?,?,?,?)",DataUser.ZLGL);
            int id = OracleOledbBase.GetMaxID("ID", string.Format("{0}.T_CHARMODIFYFIELDDICT",DataUser.ZLGL));
        ...
        {
            string str = string.Format("select maxlen from {0}.T_CHARMODIFYFIELDDICT where id=" + fieldDefineID,DataUser.ZLGL);
            int maxlen = Convert.ToInt32(OracleOledbBase.ExecuteScalar(str).ToString());
            if (charInfo.MaxLen < maxlen)
                charInfo.MaxLen = maxlen;
            string sql;
            sql = string.Format("UPDATE {0}.T_CHARMODIFYFIELDDICT SET ValueRequired =?, MultiLine = ?, DisplayLine =?, MaxLen =?, Common =?, DefaultValue = ? WHERE (ID = ?)",DataUser.ZLGL);
        ...
        {
            string sql;
            sql = string.Format("SELECT ID, ValueRequired, MultiLine, DisplayLine, MaxLen, Common, DefaultValue FROM {0}.T_CHARMODIFYFIELDDICT WHERE (ID = ?)",DataUser.ZLGL);
