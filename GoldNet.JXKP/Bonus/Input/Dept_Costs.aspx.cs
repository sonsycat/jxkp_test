...
{
    public partial class Dept_Costs : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetAccountDeptcosts(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("Dept_Costs");
            Session["Dept_Costs"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Dept_Costs"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Dept_Costs"];
