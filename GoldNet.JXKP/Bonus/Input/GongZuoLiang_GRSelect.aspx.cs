...
{
    public partial class GongZuoLiang_GRSelect : PageBase
    {
        ...
        private void data(string stardate, string enddate)
        {
            ...
            DataTable dt = report_dal.GetAccountDeptGR_GZL(stardate, enddate, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("GongZuoLiang_GRSelect");
            Session["GongZuoLiang_GRSelect"] = dt;

        }
        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["GongZuoLiang_GRSelect"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["GongZuoLiang_GRSelect"];
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    else if (dt.Columns[i].ColumnName == "USER_NAME")
                    {
                        dt.Columns[i].ColumnName = "开单医生姓名";
                    }


                }
                ex.ExportToLocal(dt, this.Page, "xls", "科室个人工作量明细");
