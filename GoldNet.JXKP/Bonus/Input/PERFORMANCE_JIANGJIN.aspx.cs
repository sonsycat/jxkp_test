...
{
    public partial class PERFORMANCE_JIANGJIN : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetAccountDept_PERFORMANCE(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("PERFORMANCE_JIANGJIN");
            Session["PERFORMANCE_JIANGJIN"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["PERFORMANCE_JIANGJIN"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["PERFORMANCE_JIANGJIN"];
