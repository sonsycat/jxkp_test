...
{
    public partial class Clinic_workload_holiday : PageBase
    {
        /// <summary>
        /// 初始化处理
        /// </summary>
        ...
        protected void Btn_Query_Click(object sender, AjaxEventArgs e)
        {
            ...
            DataTable dt = dal.GetClinicWorkloadHoliday(GetBeginDate(), "");
            if (dt != null)
            {
                SReport.DataSource = dt;
                SReport.DataBind();
                Session.Remove("Clinic_workload_holiday");
                Session["Clinic_workload_holiday"] = dt;
            }
            else
            {
        ...
        {
            if (Session["Clinic_workload_holiday"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Clinic_workload_holiday"];
                ...
                ex.ExportToLocal(dt, this.Page, "xls", "门诊双休日工作量同期(" + dates + ")");
            }
        }
