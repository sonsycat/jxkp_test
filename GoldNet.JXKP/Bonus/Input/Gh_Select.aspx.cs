...
{
    public partial class Gh_Select : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetAccountDeptGGH(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("Gh_Select");
            Session["Gh_Select"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Gh_Select"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Gh_Select"];
