<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GangGan.aspx.cs" Inherits="GoldNet.JXKP.Bonus.Input.GangGan" %>
    ...
    <script type="text/javascript">
        ...
        */
        function TreeOpration(optype) {
            if (optype == "1") {
                ...
                txtCode.setValue(this.Store1.getAt(RowIndex).get('BS'));
                txtName.setValue(this.Store1.getAt(RowIndex).get('COSTS'));
                TextField1.setValue(this.Store1.getAt(RowIndex).get('ZCBL'));
                TextField2.setValue(this.Store1.getAt(RowIndex).get('CCBL'));
                Btn_BatStart.setText("修改");
                arcEditWindow.show();
            } else if (optype == "3") {
                Btn_BatStart.setText("");
                Ext.Msg.confirm("删除项目", "确定要删除该项目：" + this.Store1.getAt(RowIndex).get('BS') + " 吗？", function (btn, text) { OpCallback(btn) });
            }
        }
        ...
        */
        function OpCallback(btn) {
            ...
            var code = txtCode.getValue();  // 标识
            var name = txtName.getValue();  // 结余额
            var a1 = TextField1.getValue(); // 正常比例
            var a2 = TextField2.getValue(); // 超出比例

            if (optype == "1") {
                GridPanelToDataBase("", code, name, optype, a1, a2);
            } else if (optype == "2") {
                GridPanelToDataBase(this.Store1.getAt(RowIndex).get('BS'), this.Store1.getAt(RowIndex).get('BS'), name, optype, a1, a2);
            } else if (optype == "3") {
                GridPanelToDataBase(this.Store1.getAt(RowIndex).get('BS'), code, "", optype, a1, a2);
            }
        }
        ...
        function GridPanelToDataBase(id, code, name, optype, a1, a2) {
            Goldnet.AjaxMethod.request(
                  'DictAjaxOper',
                    {
                        params: {
                            DictCode: code, DictName: name, OperType: optype, Subsidy: a1, Sortno: a2
                        },
...
<body>
    ...
    <ext:Store ID="Store1" runat="server" OnRefreshData="Data_RefreshData">
        <Reader>
            <ext:JsonReader>
                <Fields>
                    <ext:RecordField Name="BS" />
                    <ext:RecordField Name="COSTS" />
                    <ext:RecordField Name="ZCBL" />
                    <ext:RecordField Name="CCBL" />
                </Fields>
            </ext:JsonReader>
        </Reader>
    </ext:Store>
                                        ...
                                        <ext:LayoutColumn ColumnWidth="1">
                                            <ext:GridPanel ID="GridPanel_Show" runat="server" StoreID="Store1" Border="false"
                                                ...
                                                <ColumnModel ID="ColumnModel1" runat="server">
                                                    <Columns>
                                                        <ext:Column Header="标识" Width="100" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="BS" DataIndex="BS" />
                                                        <ext:Column Header="杠杆基础值" Width="150" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="COSTS" DataIndex="COSTS" />
                                                        <ext:Column Header="正常比例" Width="150" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="ZCBL" DataIndex="ZCBL" />
                                                        <ext:Column Header="超出比例" Width="150" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="CCBL" DataIndex="CCBL" />
                                                    </Columns>
                                                </ColumnModel>
                                                <SelectionModel>
    ...
    <ext:Window ID="arcEditWindow" runat="server" Icon="Group" Title="设置杠杆字典" Width="250"
        ...
        <Body>
            <table style="margin: 10px,10px,10px,10px">
                <tr>
                    ...
                    <td align="left">
                        <ext:TextField runat="server" ID="txtCode" Width="120">
                        </ext:TextField>
                    </td>
                </tr>
                <tr>
