...
{
    public partial class Churuku_Select : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetChuruku_Seclect(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("Churuku_Select");
            Session["Churuku_Select"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Churuku_Select"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Churuku_Select"];
