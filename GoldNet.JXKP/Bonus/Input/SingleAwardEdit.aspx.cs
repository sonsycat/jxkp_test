...
namespace GoldNet.JXKP
{
    ...
    ///  单项奖项录入编辑界面
    /// </summary>
    public partial class SingleAwardEdit : PageBase
    {
            ...
            if (!Ext.IsAjaxRequest)
            {
                ...
                if (Request["SingleAwardMode"] != null)
                {
                    ...
                    if (mode == "Edit" && Request["SingleAwardID"] != null)
                    {
                        ...
                        DataTable dt = inputSingleAward.GetSingleAwardSimple(id);
                        if (dt.Rows.Count > 0)
                        {
        ...
        protected void TypeSelect(object sender, AjaxEventArgs e)
        {
            ...
            DataTable dtType = inputSingleAward.GetSimpleType(id);
        ...
        protected void SaveEditSingleAward(object sender, AjaxEventArgs e)
        {
            ...
            if (Request["SingleAwardMode"] != null)
            {
                ...
                {//新建单项奖励数据，插入的单项奖励表中
                    ...
                    {
                        User user = (User)Session["CURRENTSTAFF"];
                        inputSingleAward.InsertSingleAward(deptname, deptid,typeid,typename,checkstan,remark,money,inputdate,user.UserName);
                ...
                {//修改单项奖励数据，更新到单项奖励表中
                    ...
                    {
                        User user = (User)Session["CURRENTSTAFF"];
                        inputSingleAward.UpdateSingleAward(id, deptname, deptid, typeid, typename, checkstan, remark, money, inputdate, user.UserName);
                        scManager.AddScript("parent.RefreshData();");
                        scManager.AddScript("parent.DetailWin.hide();");
                        scManager.AddScript("parent.DetailWin.clearContent();");
                    }
