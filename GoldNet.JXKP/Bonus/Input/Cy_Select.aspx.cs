...
{
    public partial class Cy_Select : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetAccountDeptCy(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("Cy_Select");
            Session["Cy_Select"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Cy_Select"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Cy_Select"];
