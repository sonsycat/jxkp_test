...
{
    public partial class ZcfSelect : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            string balance = cbbType.SelectedItem.Value;
            DataTable dt = report_dal.GetAccountDeptZCF(date, dept, balance);
           
            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("ZcfSelect");
            Session["ZcfSelect"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["ZcfSelect"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["ZcfSelect"];
