...
{
    public partial class WorkloadSet_Other : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Ext.IsAjaxRequest)
            {
                ...
                if (Request["item_code"] != null)
                {
                    ...
                    this.Store2.DataSource = prodal.GetNoCheckDeptByitemleibie(Request["item_code"].ToString(), Request["TYPE_CODE"].ToString()).Tables[0];
                    this.Store2.DataBind();
                    this.Store1.DataSource = prodal.GetNoCheckDeptByitemallleibie(Request["TYPE_CODE"].ToString()).Tables[0];
        ...
        public void SetDict()
        {
            ...
            //DataRow[] depttyperow = dal.GetDeptTypeleibie().Tables[0].Select();
            //foreach (DataRow rw in depttyperow)
            //{
            //    this.Combo_SelectDept.Items.Add(new Goldnet.Ext.Web.ListItem(rw["ITEM_NAME"].ToString(), rw["ITEM_CLASS"].ToString()));
            //}
            Goldnet.Dal.Appor_Prog_Dict prodal = new Goldnet.Dal.Appor_Prog_Dict();
            DataRow[] rolerow = prodal.GetIncomitemliebie().Tables[0].Select();
            foreach (DataRow row in rolerow)
            {
                this.ComboBox_Role.Items.Add(new Goldnet.Ext.Web.ListItem(row["item_name"].ToString(), row["item_code"].ToString()));
            }
        ...
        {
            Goldnet.Dal.Appor_Prog_Dict prodal = new Goldnet.Dal.Appor_Prog_Dict();
            this.Store1.DataSource = prodal.GetNoCheckDeptByitemleibie(Request["item_code"].ToString(), Request["TYPE_CODE"].ToString());
            this.Store1.DataBind();
        }
        /// <summary>
        /// 保存按钮
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void leibieData(object sender, StoreSubmitDataEventArgs e)
        {
            List<PageModels.liebieselected> leibielist = e.Object<PageModels.liebieselected>();
            Goldnet.Dal.Appor_Prog_Dict dal = new Goldnet.Dal.Appor_Prog_Dict();
            dal.insertleibie(leibielist, Request["item_code"].ToString(), Request["TYPE_CODE"].ToString());
            dal.deleteleibie(leibielist, Request["item_code"].ToString(),Request["TYPE_CODE"].ToString());
            dal.addleibie(leibielist, Request["item_code"].ToString(), Request["TYPE_CODE"].ToString());
            dal.deleteleibie_other(leibielist, Request["item_code"].ToString(), Request["TYPE_CODE"].ToString());
            this.SaveSucceed();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void SelectedDept1(object sender, AjaxEventArgs e)
        {
            Goldnet.Dal.Appor_Prog_Dict prodal = new Goldnet.Dal.Appor_Prog_Dict();
            this.Store1.DataSource = prodal.GetNoCheckDeptByitemallleibie(Request["TYPE_CODE"].ToString()).Tables[0];
            this.Store1.DataBind();
