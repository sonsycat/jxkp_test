...
{
    public partial class Dept_cost : PageBase
    {
        ...
        private void BindDate(string date)
        {
            ...
            DataTable dt = report_dal.GetAccountDept_costs(date, dept);

            Store1.DataSource = dt;
            Store1.DataBind();
            Session.Remove("Dept_cost");
            Session["Dept_cost"] = dt;

        }

        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Dept_cost"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["Dept_cost"];
