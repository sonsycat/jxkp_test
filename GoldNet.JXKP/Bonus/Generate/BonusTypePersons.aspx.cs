...
{
    public partial class BonusTypePersons : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Ext.IsAjaxRequest)
            {
                ...
                {
                    this.personstype.Items.Insert(j, new Goldnet.Ext.Web.ListItem(tb.Rows[j]["BONUS_PSERSONS_TYPE"].ToString(), tb.Rows[j]["BONUS_PSERSONS_TYPE"].ToString()));
                }
        ...
        protected void OutExcel(object sender, EventArgs e)
        {
            ...
            DataTable table = calculateBouns.GetPersonTypeBonus(this.cbbYear.SelectedItem.Value.ToString() + this.cbbmonth.SelectedItem.Value.ToString(), this.ccbYearTo.SelectedItem.Value.ToString() + this.ccbMonthTo.SelectedItem.Value.ToString(),this.personstype.SelectedItem.Text);
            ...
            for (int i = 0; i < table.Columns.Count; i++)
            {
                ...
                else if (!table.Columns[i].ColumnName.Equals("姓名"))
                {
            ...
            table.Rows.Add(dr);
            this.outexcel(table, "各类人员统计表");
        }
        private void data(string starmonth, string endmonth)
        {
            CalculateBonus calculateBouns = new CalculateBonus();

            DataTable table = calculateBouns.GetPersonTypeBonus(starmonth, endmonth, this.personstype.SelectedItem.Text);
            DataRow dr = table.NewRow();
            for (int i = 0; i < table.Columns.Count; i++)
            {
