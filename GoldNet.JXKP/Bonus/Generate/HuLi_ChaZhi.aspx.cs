...
{
    public partial class HuLi_ChaZhi : System.Web.UI.Page
    {
        /// <summary>
        /// 初始化页面
        /// </summary>
        ...
        {
            if (Session["HuLi_ChaZhi"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["HuLi_ChaZhi"];

                ex.ExportToLocal(dt, this.Page, "xls", "护理奖金汇总差值");
        ...
        protected void Btn_Search_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = calculateBonus_dal.SearchHuLi_ChaZhi(years, months, benginDate);
            if (dt.Rows.Count > 0)
            {//找到时绑定数据源
                SSearch.DataSource = dt;
                SSearch.DataBind();
                Session.Remove("Bonus_ChaZhi");
                Session["Bonus_ChaZhi"] = calculateBonus_dal.SearchHuLi_ChaZhi(years, months, benginDate);
            }
            else
            {
