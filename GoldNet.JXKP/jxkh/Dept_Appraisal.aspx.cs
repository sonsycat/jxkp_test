...
{
    public partial class Dept_Appraisal : PageBase
    {
        /// <summary>
        /// 初始化处理
        /// </summary>
        ...
        protected void Btn_Eval_Click(object sender, AjaxEventArgs e)
        {
            ...
            Dictionary<string, string>[] Depts = JSON.Deserialize<Dictionary<string, string>[]>(selectedunit);
            Dictionary<string, string>[] Guides = JSON.Deserialize<Dictionary<string, string>[]>(selectedguide);
            if (Depts.Length.Equals(0) || Guides.Length.Equals(0))
            {
                showMsg("请选择评价科室和评价指标！");
                return;
            }
            ...
            rtn = EvalAppraisal(startdate, enddate, evalapp, Depts, Guides);
            if (rtn.Equals(""))
            {
        ...
        /// <returns></returns>
        public string EvalAppraisal(string startdate, string enddate, string evalapp, Dictionary<string, string>[] Depts, Dictionary<string, string>[] Guides)
        {
            string rtn = "";
            string guidestr = "";
            string qzstr = "";
            ...
            for (int i = 0; i < Guides.Length; i++)
            {
                ...
                string strname = Guides[i]["text"];
                int str = Guides[i]["text"].LastIndexOf('(');
            ...
            DataTable table = dal.GetAppraisalByGuide(Depts, Guides, "K", incount);
            ...
            double[,] guidearr = new double[Depts.Length, Guides.Length];
            for (int i = 0; i < Guides.Length; i++)
            {
            ...
            double[] values = new double[Depts.Length];
            ...
            Session.Add("EvalDeptTable", table);
            Session.Remove("EvalDeptSetting");
            Session.Add("EvalDeptSetting", startdate + "," + enddate + "," + evalapp + "," + incount + "," + "K");
            return rtn;
        }
        ...
        //选择评价对像
        protected void Btn_SelectDept_Click(object sender, AjaxEventArgs e)
        {
            LoadConfig loadcfg = getLoadConfig("Eval_Dept_Selector.aspx");
            //获取科室权限
            string deptFilter = DeptFilter("dept_code");
            loadcfg.Params.Add(new Goldnet.Ext.Web.Parameter("deptFilter", deptFilter));
            showDetailWin(loadcfg, "选择评价科室", "760");
        }
        ...
        protected void Btn_SelectGuide_Click(object sender, AjaxEventArgs e)
        {
            ...
            loadcfg.Params.Add(new Goldnet.Ext.Web.Parameter("organ", "K"));
