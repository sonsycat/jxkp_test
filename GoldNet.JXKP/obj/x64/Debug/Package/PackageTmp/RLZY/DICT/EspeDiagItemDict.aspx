<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EspeDiagItemDict.aspx.cs"
    Inherits="GoldNet.JXKP.RLZY.DICT.EspeDiagItemDict" %>
    ...
    <script type="text/javascript">
        ...
        */
        function TreeOpration(optype) {
            if (optype == "1") {
                ...
                txtCode.setValue(this.Store1.getAt(RowIndex).get('DIAG_CODE'));
                txtName.setValue(this.Store1.getAt(RowIndex).get('DIAG_NAME'));
                Btn_BatStart.setText("修改");
                arcEditWindow.show();
            } else if (optype == "3") {
                Btn_BatStart.setText("");
                Ext.Msg.confirm("删除项目", "确定要删除该项目：" + this.Store1.getAt(RowIndex).get('DIAG_NAME') + " 吗？", function(btn, text) { OpCallback(btn) });
            } 
        }
        ...
        */
        function OpCallback (btn) {
            ...
            if (optype == "1") {
               GridPanelToDataBase("",code,name,optype);
            } else if (optype == "2") {
               GridPanelToDataBase(this.Store1.getAt(RowIndex).get('SERIAL_NO'), code,name,optype);
            } else if (optype == "3") {
               GridPanelToDataBase(this.Store1.getAt(RowIndex).get('SERIAL_NO'),"","",optype);
            }
        }
        
        function GridPanelToDataBase(id,code,name,optype) {
          Goldnet.AjaxMethod.request(
                  'DictAjaxOper',
                    {
                        params: {
                            DictCode:id,DictName:name,OperType:optype,Code:code
                        },
...
<body>
        ...
        <Reader>
            <ext:JsonReader>
                <Fields>
                    ...
                    <ext:RecordField Name="DIAG_CODE" />
                    <ext:RecordField Name="DIAG_NAME" />
                </Fields>
            </ext:JsonReader>
        </Reader>
    </ext:Store>
                                        ...
                                        <ext:LayoutColumn ColumnWidth="1">
                                            <ext:GridPanel ID="GridPanel_Show" runat="server" StoreID="Store1" Border="false"
                                                ...
                                                <ColumnModel ID="ColumnModel1" runat="server">
                                                    <Columns>
                                                        <ext:Column Header="代码" Width="150" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="DIAG_CODE" DataIndex="DIAG_CODE" />
                                                        <ext:Column Header="名称" Width="150" Align="Center" Sortable="false" MenuDisabled="true"
                                                            ColumnID="DIAG_NAME" DataIndex="DIAG_NAME" />
                                                    </Columns>
                                                </ColumnModel>
                                                <SelectionModel>
    ...
    <ext:Window ID="arcEditWindow" runat="server" Icon="Group" Title="设置类别字典" Width="250"
        Height="110" AutoShow="false" Modal="true" CenterOnLoad="true" ShowOnLoad="false"
