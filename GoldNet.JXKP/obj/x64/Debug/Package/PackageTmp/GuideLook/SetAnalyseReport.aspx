<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SetAnalyseReport.aspx.cs" Inherits="GoldNet.JXKP.GuideLook.SetAnalyseReport" %>
        ...
        */
        function nodeOpCallback (btn, text, optype, node) {
            ...
            if (optype == "1") {
                ...
                if(this.Store1.getCount() > 0) {
                    sortNum = this.Store1.getAt(this.Store1.getCount() - 1).get('RPT_RANKING') + 1;
                }
                GridPanelToDataBase(text, optype, node,"","","",sortNum);
            } else if (optype == "2") {
        ...
        function GridPanelToDataBase(text, optype, node,reportid,TempId,TempRanking,SortNum) {
         ...
         if(optype == "4" || optype == "5") {
            GridPanel_Show.el.mask('正在加载...', 'x-loading-mask');
         }
          var row = rowselection;
          var selections = row.getSelections();
...
<body>
    ...
    <ext:Window ID="arcEditWindow" runat="server" Icon="Group" Title="设置报表指标" Width="775"
    ...
    <ext:Window ID="ViewWindow" runat="server" Icon="Group" Title="预览分析报表" Width="775"
        Height="385" AutoShow="false" Modal="true" CenterOnLoad="true" AutoScroll="true"
        ShowOnLoad="false" Resizable="false" StyleSpec="background-color:Transparent;"
