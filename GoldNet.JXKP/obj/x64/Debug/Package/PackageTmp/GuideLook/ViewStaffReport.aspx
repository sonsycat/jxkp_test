<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ViewStaffReport.aspx.cs"
    Inherits="GoldNet.JXKP.GuideLook.ViewStaffReport" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">                   
       var cellSelect = function (grid, rowIndex, colIndex,MenuItem3,MenuItem1,MenuItem2) {
            var record = grid.store.getAt(rowIndex);
            var name = grid.getColumnModel().getDataIndex(colIndex);
            var value = record.get(name);
            var deptCode = record.get('STAFF_ID');
            var StaffName = record.get('NAME');
            ...
            Goldnet.AjaxMethod.request(
                  'MenuItemJuge',
                    {
                    params: {
                        GuideCode:GuideCode,Staffid:deptCode
                    },
                    success: function(result) {
                        ...
                        var path = 'DEPT_CODE='+deptCode+'&GuideCode='+GuideCode+'&Name='+StaffName;
    ...
    <ext:Menu ID="ContextMenu" runat="server">
        <Items>
            <ext:MenuItem ID="MenuItem3" runat="server" Icon="NoteEdit" Text="查看明细数据" Disabled="true">
                <AjaxEvents>
                    <Click OnEvent="StaffGuideDetail">
                        <ExtraParams>
            ...
            <ext:MenuItem ID="MenuItem1" runat="server" Icon="NoteEdit" Text="按月分析指标值" Disabled="true">
                <AjaxEvents>
                    <Click OnEvent="StaffByMonthGuide">
                        <ExtraParams>
                            <ext:Parameter Name="NodeId" Value="Ext.encode(#{TreeCtrl}.getSelectionModel().getSelectedNode().id)"
                                Mode="Raw">
                            </ext:Parameter>
            ...
            <ext:MenuItem ID="MenuItem2" runat="server" Icon="NoteEdit" Text="查看相关性信息" Disabled="true">
                <AjaxEvents>
                    <Click OnEvent="SatffGuideRelation">
                        <ExtraParams>
                            <ext:Parameter Name="NodeId" Value="Ext.encode(#{TreeCtrl}.getSelectionModel().getSelectedNode().id)"
                                Mode="Raw">
                            </ext:Parameter>
                    ...
                    <Center>
                        <ext:GridPanel ID="GridPanel_Show" runat="server" Border="false" StoreID="Store1"
                            ...
                            <Listeners>
                                ...
                                <CellContextMenu Handler="cellSelect(this,rowIndex,cellIndex,#{MenuItem3},#{MenuItem1},#{MenuItem2})" />
                                <MouseOver Fn="MouseGrid" />
                            </Listeners>
                        </ext:GridPanel>
                    </Center>
                    <West Collapsible="false" Split="false" CollapseMode="Mini">
                        <ext:Panel ID="Panel2" runat="server" Width="200" BodyBorder="false" Title="请选择报表"
