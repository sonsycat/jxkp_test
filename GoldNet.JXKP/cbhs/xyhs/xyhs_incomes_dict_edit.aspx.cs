...
{
    public partial class xyhs_incomes_dict_edit : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Ext.IsAjaxRequest)
            {
                if (Request["itemcode"].ToString() != "")
                {
                    XyhsDetail dal = new XyhsDetail();
                    DataTable tb = dal.GetxyhsincomesList(Request["itemcode"].ToString()).Tables[0];
                    if (tb.Rows.Count > 0)
                    {
                        this.Combo_ItemType.SelectedItem.Text = tb.Rows[0]["flags"].ToString();
                        this.Text_ItemName.Text = tb.Rows[0]["ITEM_NAME"].ToString();
                        this.ratio.Text = tb.Rows[0]["RATIO"].ToString();
                    }
                }
