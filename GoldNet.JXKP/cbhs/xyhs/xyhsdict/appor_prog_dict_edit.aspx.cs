...
namespace GoldNet.JXKP.cbhs.xyhs.xyhsdict
{
    ...
    {
        private XyhsOperation dal = new XyhsOperation();
            ...
            if (!Ext.IsAjaxRequest)
            {
                if (Request["ApproProgEditMode"] != null)
                {
                    ...
                    if (mode == "Edit" && Request["ApporProgID"] != null)
                    {
                        ...
                        DataTable dt = dal.GetProgDict(id);
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            this.ComboBox1.SetValue(dt.Rows[0]["PROG_EXPRESS"].ToString());
                            this.ComboBox1.SelectedItem.Value = dt.Rows[0]["PROG_EXPRESS"].ToString();
                        }
                    }
                    else if (mode == "Add")
                    {
                        ...
                        tfINPUTCODE.Text = "";
                        ccbtype.SetValue("");
                        this.ComboBox1.SetValue("");
                    }
                    Store2.DataSource = dal.GetApporDict();
                    Store2.DataBind();
                    Store3.DataSource = dal.GetGuidesqlDict();
                    Store3.DataBind();
                }
            }
        }
        ...
        protected void SaveProg_onClick(object sender, AjaxEventArgs e)
        {
            if (Request["ApproProgEditMode"] != null)
            {
                ...
                string guidecode = this.ComboBox1.SelectedItem.Value;
                if (mode == "Add")
                {
                    try
                    {
                        dal.InsertProgDict(progcode, progname, inputcode, type,guidecode);
                ...
                else if (mode == "Edit")
                {
                    ...
                    try
                    {
                        dal.UpateProgDict(id, progcode, progname, inputcode, type,guidecode);
                        Goldnet.Ext.Web.ScriptManager scManager = Goldnet.Ext.Web.ScriptManager.GetInstance(this.Page);
