...
{
    public partial class patient_account : PageBase
    {
        ...
        protected void Btn_New_Add(object sender, EventArgs e)
        {
            ...
            Ext.Msg.Confirm("系统提示", "您确定要生成-" + date + "-的数据吗？<br>如果这个月的数据存在会覆盖的，这个得确定好！", new MessageBox.ButtonsConfig
            {
        ...
        {
            string months = this.cbbBeginMonth.SelectedItem.Value.Length == 1 ? "0" + this.cbbBeginMonth.SelectedItem.Value : this.cbbBeginMonth.SelectedItem.Value;
            string date_time = this.cbbBeginYear.SelectedItem.Value + months;
            ...
            try
            {
                ...
                dal.Exec_sp_xyhs_patient_cost(date_time);
                this.Session["LongActionProgress"] = "2:正在生成数据(2/2)";
                dal.Exec_sp_patien_account(date_time);
