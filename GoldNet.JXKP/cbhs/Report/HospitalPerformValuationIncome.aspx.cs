...
{
    public partial class HospitalPerformValuationIncome : PageBase
    {
        ...
        private void Query()
        {
            ...
            DataTable table = report_dal.GetHospitalPerformValuationIncome(GetBeginDate(), GetEndDate(), balance, deptcode, depttype);
            if (table != null && table.Rows.Count > 0)
            {
                ...
                Session.Remove("HospitalPerformValuationIncome");
                Session["HospitalPerformValuationIncome"] = STable;
            }
            else
            {
        ...
        {
            if (Session["HospitalPerformValuationIncome"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["HospitalPerformValuationIncome"];
                ...
                ex.ExportToLocal(dt, this.Page, "xls", "住院执行科室计价收入统计表");
            }
        }
        /// <summary>
        /// 开始时间
