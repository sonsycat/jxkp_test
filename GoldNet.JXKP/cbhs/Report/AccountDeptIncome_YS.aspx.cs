...
{
    public partial class AccountDeptIncome_YS : PageBase
    {
        ...
        protected void Btn_Query_Click(object sender, AjaxEventArgs e)
        {
            ...
            string incomestype = this.incomestype.SelectedItem.Value == "" ? "1" : this.incomestype.SelectedItem.Value;
            //默认不包含中心收入
            string centerincome = this.ComboBox1.SelectedItem.Value == "" ? "1" : this.ComboBox1.SelectedItem.Value;

            DataTable dt = report_dal.GetAccountDeptIncomeYS_new(GetBeginDate(), GetEndDate(), balance, dept, incomestype, centerincome);
            if (dt != null)
            {
        ...
        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["Account_Dept_Income"] != null)
            {
                ...
                ISheet sheet = workbook.CreateSheet("核算单位医疗收入统计报表（核算前）");
                ...
                using (MemoryStream exportData = new MemoryStream())
                {
                    ...
                    {
                        Response.AddHeader("content-disposition", "attachment; filename=" + s_date + "核算单位医疗收入统计报表（核算前）.xls");
                    }
                    else
                    {
                        Response.AddHeader("content-disposition", "attachment; filename=" + s_date + "至" + e_date + "核算单位医疗收入统计报表（核算前）.xls");
                    }
