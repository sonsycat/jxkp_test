...
{
    public partial class AccountDeptMonthPureIncome : PageBase
    {
        ...
        protected void Btn_Query_Click(object sender, AjaxEventArgs e)
        {
            ...
            DataTable dt = dal_report.GetAccountDeptMonthPurIncome(dtBegin, balance, deptcode, months + 1);
            if (dt != null)
            {
                ...
                SReport.DataBind();
                Session.Remove("AccountDeptMonthPureIncome");
                Session["AccountDeptMonthPureIncome"] = dt;
            }
            else
            {
                Goldnet.Ext.Web.Ext.Msg.Show(new Goldnet.Ext.Web.MessageBox.Config
        ...
        {
            if (Session["AccountDeptMonthPureIncome"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["AccountDeptMonthPureIncome"];
                dt.Columns.Remove("DEPT_CODE");
                ex.ExportToLocal(dt, this.Page, "xls", "月份各单位纯收入表");
            }
        }
        /// <summary>
        /// 开始时间
