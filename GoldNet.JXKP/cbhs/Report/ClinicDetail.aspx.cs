...
{
    public partial class ClinicDetail : PageBase
    {
        ...
        protected void Btn_Query_Click(object sender, AjaxEventArgs e)
        {
            ...
            DataTable dt = report_dal.GetClinicDetail(GetBeginDate(), GetEndDate(), dept, cbbType.SelectedItem.Value);
            if (dt != null)
            {
        ...
        protected void OutExcel(object sender, EventArgs e)
        {
            if (Session["ClinicDetail"] != null)
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    else if (dt.Columns[i].ColumnName == "WNUM")
                    {
                        dt.Columns[i].ColumnName = "工作日工作量";
                    }
                    else if (dt.Columns[i].ColumnName == "HNUM")
                    {
                        dt.Columns[i].ColumnName = "双休日工作量";
                    }
                    else if (dt.Columns[i].ColumnName == "W_NUMBERS")
                    {
                        dt.Columns[i].ColumnName = "工作日标准";
                    }
                    else if (dt.Columns[i].ColumnName == "H_NUMBERS")
                    {
                        dt.Columns[i].ColumnName = "双休日标准";
                    }
