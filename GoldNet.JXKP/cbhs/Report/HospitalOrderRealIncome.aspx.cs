...
{
    public partial class HospitalOrderRealIncome : PageBase
    {
        ...
        private void Query()
        {
            ...
            DataTable table = report_dal.GetHospitalOrderrealIncome(GetBeginDate(), GetEndDate(), balance, deptcode, depttype);
            if (table != null && table.Rows.Count>0)
            {
                ...
                Session.Remove("HospitalOrderRealIncome");
                Session["HospitalOrderRealIncome"] = STable;
            }
            else
            {
        ...
        {
            if (Session["HospitalOrderRealIncome"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["HospitalOrderRealIncome"];
                ...
                ex.ExportToLocal(dt, this.Page, "xls", "住院开单科室实际收入统计表");
            }
        }
        /// <summary>
        /// 开始时间
        ...
        protected void DbRowClick(object sender, AjaxEventArgs e)
        {
            ...
            else
            {
                ...
                loadcfg.Params.Add(new Goldnet.Ext.Web.Parameter("groupbydept", "ORDERED_BY_DEPT"));//开单科室
                loadcfg.Params.Add(new Goldnet.Ext.Web.Parameter("incomescolums", "charges"));//计价0/实收1
