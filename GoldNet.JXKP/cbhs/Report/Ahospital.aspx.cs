...
{
    public partial class Ahospital : PageBase
    {
        ...
        private void data(string stdate, string enddate)
        {
            ...
            DataTable table = report_dal.GetAHospitalsys(stdate, enddate, deptcode);
            ...
            for (int i = 0; i < table.Columns.Count; i++)
            {
                ...
                else if (table.Columns[i].ColumnName == "WARD_NAME")
                {
                    cl.Header = "护理单元";
                }
                else if (table.Columns[i].ColumnName == "PERSONS")
                {
                    cl.Header = "床日数";
                }
                cl.Sortable = false;
