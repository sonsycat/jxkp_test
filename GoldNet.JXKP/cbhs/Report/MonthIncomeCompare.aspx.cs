...
{
    public partial class MonthIncomeCompare : PageBase
    {
        ...
        protected void Btn_Query_Click(object sender, AjaxEventArgs e)
        {
            ...
            DataTable dt = dal_report.GetMonthIncomeCompare(GetBeginDate(), GetEndDate(), balance, deptcode, hospro);
            if (dt != null)
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    if (dt.Columns[i].ColumnName != "DEPT_CODE")
                    {
                        ...
                        if (dt.Columns[i].ColumnName != "月份" && dt.Columns[i].ColumnName != "收入类别")
                        {
                ...
                SReport.DataBind();
                Session.Remove("MonthIncomeCompare");
                Session["MonthIncomeCompare"] = dt;
            }
            else
            {
        ...
        {
            if (Session["MonthIncomeCompare"] != null)
            {
                ExportData ex = new ExportData();
                DataTable dt = (DataTable)Session["MonthIncomeCompare"];
                                
                ex.ExportToLocal(dt, this.Page, "xls", "收入按月份对比表");
            }
        }
        /// <summary>
        /// 开始时间
