...
namespace GoldNet.Comm.DAL.Oracle 
{
	...
	public abstract class OracleBase
	{
		...
		/// oracle连接
		/// </summary>
        public static string ConnString = System.Configuration.ConfigurationManager.AppSettings["OracleConnString"];
        public static string OLEDBConnString = System.Configuration.ConfigurationManager.AppSettings["OracleOLEDBConnString"];
...
//            connstr.AppendFormat(@"Provider=MSDAORA.1;Data Source=(DESCRIPTION =  
//                                                                    (ADDRESS_LIST =  
...
//                                                                      (SERVER = DEDICATED)  
//                                                                    )  
//                                                                  );User Id=comm;Password=comm;");
            connstr.AppendFormat(@"Provider=OraOLEDB.Oracle;
            Data Source=(DESCRIPTION=(CID=GTU_APP)(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=127.0.0.1)(PORT=1521)))(CONNECT_DATA=(SID=jxkp)(SERVER=DEDICATED)));
            User Id=comm;Password=comm; ");
            using (OleDbConnection connection = new OleDbConnection(OLEDBConnString.ToString()))
            {
                DataSet ds = new DataSet();
                try
                {
		...
		/// <param name="SQLString">SQL语句</param>
		/// <returns>影响的记录数</returns>
		public static void ExecuteSql(string SQLString,params OracleParameter[] cmdParms)
		{
            using (OracleConnection connection = new OracleConnection(ConnString))
			{				
        ...
        public static object GetSingle(string conn,string SQLString, params OracleParameter[] cmdParms)
        {
        ...
        /// <returns></returns>
        public static void RunProcedure(string conn,string storedProcName, IDataParameter[] parameters)
        {
